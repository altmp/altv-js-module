...
const custom_getPromiseDetails = async (promise) => {
  const fullfiledPromise = Promise.resolve()
  const state = await Promise.race([promise, fullfiledPromise]).then(
    (p) => (p === fullfiledPromise ? kPending : kFulfilled),
    () => kRejected
  )
...
getPromiseDetails = custom_getPromiseDetails

const custom_getProxyDetails = () => undefined

getProxyDetails = custom_getProxyDetails
...
const {
  ...
  MapPrototypeGetSize = Object.getOwnPropertyDescriptor(Map.prototype, "size")
    .get.call,
  ...
  SetPrototypeGetSize = Object.getOwnPropertyDescriptor(Set.prototype, "size")
    .get.call,
  ...
  TypedArrayPrototypeGetSymbolToStringTag = Object.getOwnPropertyDescriptor(
    TypedArrayPrototype,
    Symbol.toStringTag
  ).get.call,
} = primordials
...
let {
  ...
  isStringObject = (obj) =>
    typeof obj === "object" && obj != null && obj.constructor === String,
  isNumberObject = (obj) =>
    typeof obj === "object" && obj != null && obj.constructor === Number,
  isBooleanObject = (obj) =>
    typeof obj === "object" && obj != null && obj.constructor === Boolean,
  isBigIntObject = (obj) =>
    typeof obj === "object" && obj != null && obj.constructor === BigInt,
...
const custom_isArgumentsObject = (obj) =>
  obj + "" === "[object Arguments]" && obj[SymbolIterator] != null
...
const custom_isModuleNamespaceObject = (obj) => {
  try {
    const descr =
      obj && Object.getOwnPropertyDescriptor(obj, Symbol.toStringTag)
    return (
      descr.value === "Module" &&
      !(descr.writable || descr.enumerable || descr.configurable)
    )
  } catch {
    return false
  }
...
async function inspect(value, opts) {
  // Default options
  const ctx = {
    budget: {},
    indentationLvl: 0,
  ...
  return await formatValue(ctx, value, 0)
}
...
async function getConstructorName(obj, ctx, recurseTimes, protoProps) {
  let firstProto
  const tmp = obj
  ...
  const protoConstr = await getConstructorName(
    firstProto,
    ctx,
    recurseTimes + 1,
    protoProps
...
async function addPrototypeProperties(ctx, main, obj, recurseTimes, output) {
  let depth = 0
  let keys
  let keySet
  do {
    ...
    for (const key of keys) {
      ...
      const value = await formatProperty(
        ctx,
        obj,
        recurseTimes,
        key,
...
async function formatProxy(ctx, proxy, recurseTimes) {
  if (recurseTimes > ctx.depth && ctx.depth !== null) {
  ...
  const res = [
    await formatValue(ctx, proxy[0], recurseTimes),
    await formatValue(ctx, proxy[1], recurseTimes),
  ]
  ctx.indentationLvl -= 2
...
async function formatValue(ctx, value, recurseTimes, typedArray) {
  ...
  if (proxy !== undefined) {
    ...
    if (ctx.showProxy) {
      return await formatProxy(ctx, proxy, recurseTimes)
    }
    value = proxy
  }
    ...
    if (
      ...
      if (ret !== context) {
        if (typeof ret !== "string") {
          return await formatValue(ctx, ret, recurseTimes)
        }
  ...
  return await formatRaw(ctx, value, recurseTimes, typedArray)
}

async function formatRaw(ctx, value, recurseTimes, typedArray) {
  let keys
  let protoProps
  ...
  const constructor = await getConstructorName(
    value,
    ctx,
    recurseTimes,
    protoProps
  ...
  try {
    // added await because of the promise details shit
    output = await formatter(ctx, value, recurseTimes)
    for (i = 0; i < keys.length; i++) {
      output.push(
        await formatProperty(ctx, value, recurseTimes, keys[i], extrasType)
      )
    }
...
async function formatNamespaceObject(keys, ctx, value, recurseTimes) {
  const output = new Array(keys.length)
  for (let i = 0; i < keys.length; i++) {
    try {
      output[i] = await formatProperty(
        ctx,
        value,
        recurseTimes,
        keys[i],
...
async function formatSpecialArray(
  ctx,
  value,
  recurseTimes,
  maxLength,
  ...
  for (; i < keys.length && output.length < maxLength; i++) {
    ...
    output.push(
      await formatProperty(ctx, value, recurseTimes, key, kArrayType)
    )
    index++
  }
...
async function formatArray(ctx, value, recurseTimes) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (!ObjectPrototypeHasOwnProperty(value, i)) {
      return await formatSpecialArray(ctx, value, recurseTimes, len, output, i)
    }
    output.push(await formatProperty(ctx, value, recurseTimes, i, kArrayType))
  }
  if (remaining > 0)
    output.push(`... ${remaining} more item${remaining > 1 ? "s" : ""}`)
  return output
...
async function formatTypedArray(value, length, ctx, ignored, recurseTimes) {
  ...
  if (ctx.showHidden) {
    ...
    for (const key of [
      ...
      const str = await formatValue(ctx, value[key], recurseTimes, true)
...
async function formatSet(value, ctx, ignored, recurseTimes) {
  const output = []
  ctx.indentationLvl += 2
  for (const v of value) {
    ArrayPrototypePush(output, await formatValue(ctx, v, recurseTimes))
  }
  ctx.indentationLvl -= 2
  return output
}

async function formatMap(value, ctx, ignored, recurseTimes) {
  const output = []
  ctx.indentationLvl += 2
  for (const { 0: k, 1: v } of value) {
    output.push(
      `${await formatValue(ctx, k, recurseTimes)} => ${await formatValue(
        ctx,
        v,
        recurseTimes
      )}`
...
async function formatSetIterInner(ctx, recurseTimes, entries, state) {
  const maxArrayLength = MathMax(ctx.maxArrayLength, 0)
  const maxLength = MathMin(maxArrayLength, entries.length)
  const output = new Array(maxLength)
  ctx.indentationLvl += 2
  for (let i = 0; i < maxLength; i++) {
    output[i] = await formatValue(ctx, entries[i], recurseTimes)
  }
  ctx.indentationLvl -= 2
...
async function formatMapIterInner(ctx, recurseTimes, entries, state) {
  const maxArrayLength = MathMax(ctx.maxArrayLength, 0)
  ...
  if (state === kWeak) {
    for (; i < maxLength; i++) {
      const pos = i * 2
      output[i] = `${await formatValue(
        ctx,
        entries[pos],
        recurseTimes
      )} => ${await formatValue(ctx, entries[pos + 1], recurseTimes)}`
    }
    // Sort all entries to have a halfway reliable output (if more entries than
    // retrieved ones exist, we can not reliably return the same output) if the
    // output is not sorted anyway.
    ...
    for (; i < maxLength; i++) {
      const pos = i * 2
      const res = [
        await formatValue(ctx, entries[pos], recurseTimes),
        await formatValue(ctx, entries[pos + 1], recurseTimes),
      ]
...
async function formatWeakSet(ctx, value, recurseTimes) {
  const entries = previewEntries(value)
  return await formatSetIterInner(ctx, recurseTimes, entries, kWeak)
}

async function formatWeakMap(ctx, value, recurseTimes) {
  const entries = previewEntries(value)
  return await formatMapIterInner(ctx, recurseTimes, entries, kWeak)
}

async function formatIterator(braces, ctx, value, recurseTimes) {
  const { 0: entries, 1: isKeyValue } = previewEntries(value, true)
  if (isKeyValue) {
    // Mark entry iterators as such.
    braces[0] = braces[0].replace(/ Iterator] {$/, " Entries] {")
    return await formatMapIterInner(ctx, recurseTimes, entries, kMapEntries)
  }

  return await formatSetIterInner(ctx, recurseTimes, entries, kIterator)
}

async function formatPromise(ctx, value, recurseTimes) {
  let output
  const { 0: state, 1: result } = await getPromiseDetails(value)
  if (state === kPending) {
    output = [ctx.stylize("<pending>", "special")]
  } else {
    ctx.indentationLvl += 2
    const str = await formatValue(ctx, result, recurseTimes)
    ctx.indentationLvl -= 2
    output = [
      state === kRejected
...
async function formatProperty(
  ...
  if (desc.value !== undefined) {
    ...
    str = await formatValue(ctx, desc.value, recurseTimes)
    ...
    if (
      ...
      try {
        ...
        if (tmp === null) {
          ...
          str = `${s(`[${label}]`, sp)} ${await formatValue(
            ctx,
            tmp,
            recurseTimes
          )}`
...
function formatWithOptions(inspectOptions, ...args) {
  if (typeof inspectOptions !== "object" || inspectOptions === null) {
    throw new Error(
      "invalid arg type inspectOptions",
      "object",
      inspectOptions
    )
  }
