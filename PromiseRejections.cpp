...
void V8::PromiseRejections::RejectedWithNoHandler(V8ResourceImpl *resource, v8::PromiseRejectMessage &data)
{
	...
	queue.push_back(std::make_unique<PromiseRejection>(isolate, data.GetPromise(),
													   data.GetValue(), V8::SourceLocation::GetCurrent(isolate)));
}
...
void V8::PromiseRejections::HandlerAdded(V8ResourceImpl *resource, v8::PromiseRejectMessage &data)
{
	...
	auto newEnd = std::remove_if(
		...
		[&](std::unique_ptr<PromiseRejection> &rejection) {
			return rejection->promise.Get(isolate) == data.GetPromise();
		});
...
V8::PromiseRejection::PromiseRejection(v8::Isolate *isolate, v8::Local<v8::Promise> _promise, v8::Local<v8::Value> _value, V8::SourceLocation &&_location) : promise(isolate, _promise), value(isolate, _value), location(std::move(_location))
{
}
